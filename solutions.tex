...
\end{exercise}
\begin{proof}
If we have a sequence $X_1,...,X_n$ of spheres, where $X_i$ is joined to $X_{i+1}\forall i\in[1,n)$, then we can collapse arcs in each of them to points to obtain $\bigvee_{i=1}^n S^2$, as shown below:

\includegraphics[scale=0.5]{Screenshot (1357).png}

If we also had $X_1$ joined to $X_n$, we would then have a space homotopy equivalent to $S^1\vee\bigvee_{i=1}^n S^2$, as shown below:

\includegraphics[scale=0.5]{Screenshot (1358).png}

Hence, if we have $n$ spheres, and they form a simple connected graph with exactly $m$ loops, then the space is homotopy equivalent to $(\bigvee_{i=1}^m S^1)\vee(\bigvee_{i=1}^n S^2)$.
\end{proof}
