...
\end{exercise}
\begin{proof}
Let $\sim_X$ be the equivalence relation on $X$ given by $a\sim_X b$ if and only if $a$ and $b$ are in the same path-component. Similarly define the relation $\sim_Y$ on $Y$. Let $g:Y\to X$ be a homotopy inverse of $f$. We let $\phi_t$ and $\psi_t$ be homotopies from $f\circ g$ to $\text{id}_Y$ and from $g\circ f$ to $\text{id}_X$ respectively.
...
\textbf{We now show that $f$ induces a surjective map on path components.}\\Let $y\in Y$. Then $y=\phi_1(y)\sim_Y \phi_0(y)=f(g(y))$, and hence $y$ is in the same path component as a point in the image of $f$.
...
Let $y\in Y$. Let $[y]_Y$ be the connected component of $y$. As show earlier, $y$ is in the same path component as $g(y)$, and hence is in the same connected component as $g(y)$. Hence, $[y]_Y$ is in the image of the map on connected components induced by $f$.
