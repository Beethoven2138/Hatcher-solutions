...
\end{exercise}
\begin{proof}
Firstly, we can represent the spheres as a simple connected graph, where each vertex represents a sphere, and two vertices being connected by an edge represents two spheres intersecting at a point. Let $T$ be a spanning tree of the graph. We can construct a CW complex of $X$ as follows: For each sphere $X_i$, let $A_i$ be the set of points of intersection of $X_i$ with other spheres which are connected to $X_i$ by an edge in $T$, and let $B_i$ be the set of points of intersection of $X_i$ with other spheres which are connected to $X_i$ by an edge not in $T$. We then form a $0$-skeleton consisting of $0$-cells which correspond to the points in $A_1\cup...\cup A_n$, and then pairs of $0$-cells, where each pair corresponds to a point in $B_1\cup...\cup B_n$. For each $i$, we connect all the points in $A_i$ together with a single point in every pair of points corresponding to $B_i$ by $1$-cells, so as to form a simple acyclic graph, which we shall denote as $G_i$. We then connect the points comprising each pair of points corresponding to $B_i$ with a $1$-cell. Finally, we attach a $2$-cell to each $G_i$, which then gives a space that is homotopy equivalent to $X$; for we can collapse the $1$-cells connecting the pairs of points in the $B_i$'s to obtain all the intersections. This is illustrated in the following diagram, where the black circles represent $2$-spheres, the green dots represent a pair in a $B_i$, the red points represent points in $A_i$, the purple lines represent $G_i$'s, and the blue line represents the line connecting the points in the $B_i$'s.

\includegraphics[scale=0.5]{Screenshot (1361).png}

If we instead collapse each $G_i$ to a point, we then obtain a wedge sum of $n$ $2$-spheres, along with the loops corresponding to the $1$-cells connecting the pairs of points in the $B_i$'s, which are now all the same point.
\end{proof}
...
Now assume that $X$ is connected. Let $l$ denote the number of loops in $X$. Each loop will become a torus in $Y$.
\end{proof}
