...
\end{exercise}
\begin{proof}
We first observe that if we have a sphere $A$, and spheres $A_1,...,A_n$ all connect to $A$ at points $a_1,...,a_n$, then we can connect the points by $1$-cells, and then collapse them all to a single point. This is shown in the following diagram, where the red dots are $a_1,...,a_n$, and the green lines are the $1$-cells.

\includegraphics[scale=0.5]{Screenshot (1354).png}

\noindent We can thus represent the spheres as a simple connected graph, where two vertices being connected by an edge represents a pair of spheres intersecting at a point. 
\end{proof}
