...
\begin{document}
\maketitle
(*) indicates an unfinished exercise.
...
\noindent Now let $\sim_X$ and $\sim_Y$ be the corresponding relations for connected components.
...
\end{exercise}
\begin{proof}
Without loss of generality, assume that none of the edges intersect at any point other than a vertex (this can be assumed, since if there is such a point of intersection, we can simply add a vertex at that point, and split the edges in two). Let $T$ be a spanning tree of $X$, and let $x_0$ be a vertex in $T$. Let $E_1,\dots, E_n$ be the closures of the edges in $X$ not contained in $T$, so that $X=T\cup E_1\dots\cup E_n$. We now observe that given any $m$, $T\cup E_1\dots\cup E_{m+1}$ has exactly one more bounded complementary region than $T\cup E_1\dots\cup E_{m}$. Hence, by induction, $X$ has exactly $n$ bounded complementary regions, which we shall denote as $X_1,\dots, X_n$. Let $p_1,\dots, p_n:I\to X$ be loops as described in the problem; that is, each $p_i$ is a loop formed by the boundary of the bounded region $X_i$, joined to $x_0$ by a path in $X$, which we in particular require to be a path in $T$. Since $T$ is contractible, we can collapse it to a point, which will then produce the wedge sum of $n$ circles, with each circle corresponding to an $E_i$. Let $q:X\to X/T$ be the quotient map, which is a homotopy equivalence. $\{q_*([p_1]),\dots,q_*([p_n])\}$ is then a basis for $X/T$. To see this, we observe that each $q_*([p_{i+1}])$ will traverse one additional circle than all the $q_*([p_1]),\dots,q_*([p_i])$'s, and furthermore that each $q_*([p_i])$ traverses a circle at most once, and hence we are able to perform appropriate concatenations to obtain loops which each traverse exactly one circle once around, which is known to be a basis. Hence, by applying $q_*^{-1}$, $\{[p_1],\dots, [p_n]\}$ is a basis for $X$, as required.
\end{proof}
...
\end{proof}
\begin{exercise}
The Klein bottle is usually pictured as a subspace of $\mathbb{R}^3$ like the subspace $X\subset\mathbb{R}^3$ shown in the first figure of the diagram. If one wanted a model that could actually function as a bottle, one would delete the open disk bounded by the circle of self-intersection of $X$, producing a subspace $Y\subset X$. Show that $\pi_1(X)\cong\mathbb{Z}*\mathbb{Z}$ and that $\pi_1(Y)$ has the presentation $\langle a,b,c\mid aba^{-1}b^{-1}cb^\epsilon c^{-1}\rangle$ for $\epsilon=\pm 1$. (Changing the sign of $\epsilon$ gives an isomorphic group, as it happens.) Show also that $\pi_1(Y)$ is isomorphic to $\pi_1(\mathbb{R}^3\setminus Z)$ for $Z$ the graph shown in the figure. The groups $\pi_1(X)$ and $\pi_1(Y)$ are not isomorphic, but this is not easy to prove; see the discussion in Example 1.B.13.

\includegraphics[scale=0.5]{Screenshot 2026-02-22 at 03-09-17 AT.dvi - AT.pdf.png}
\end{exercise}
\begin{proof}
By Exercise $0.20$, we have that $X$ is homotopy equivalent to $S^1\vee S^1\vee S^2$, and so has fundamental group $\mathbb{Z}*\mathbb{Z}$ by Proposition $1.26$. 

\noindent The left cell complex on the diagram is $Y$, and the right cell complex $Y'$ is a deformation retract of $Y$.

\includegraphics[scale=0.5]{Screenshot (2285).png}

\noindent $Y^1$ is simply $S^1\vee S^1\vee S^1$, and the attaching map of the single $2$-cell is given by $aba^{-1}b^{-1}cb^{-1}c^{-1}$, and hence $\pi_1(Y)\cong\langle a,b,c\mid aba^{-1}b^{-1}cb^{-1}c^{-1}\rangle$, as required.

\noindent Finally, we observe that $Y$ is a deformation retract of $\mathbb{R}^3\setminus Z$, and hence $\pi_1(Z)\cong\pi_1(\mathbb{R}^3\setminus Z)$.
\end{proof}
\begin{exercise}
...
\end{exercise}
\begin{proof}
We first note that $\pi_1(Y)^\text{ab}\cong \mathbb{Z}^2$, since $[aba^{-1}b^{-1}cb^\epsilon c^{-1}]=[b^\epsilon]=0\in\pi_1(Y)^\text{ab}$, and so $[b]=0$.

\noindent Let $Z$ be the CW complex shown below.

\includegraphics[scale=0.4]{Screenshot (2286).png}

\noindent Proposition $1.26$ then gives $\pi_1(Z)=\langle a,b,c\mid abab^{-1}cac^{-1}\rangle$, and hence $\pi_1(Z)^\text{ab}\cong \mathbb{Z}_3\times\mathbb{Z}^2$, which is clearly not isomorphic to $\pi_1(Y)^\text{ab}$. Hence, $\pi_1(Y)\not\cong\pi_1(Z)$.
\end{proof}
\begin{exercise}
...
\end{exercise}
\begin{proof}
The diagram below shows the identifications made to $I^3$ to give $X$. From the diagram, we see that there are two $0$-cells (the orange and burgundy crosses), four $1$-cells (the yellow, cyan, burgundy and orange circles), three $2$-cells (the green, red and burgundy arrows), and one $3$-cell (the interior of the cube).

\includegraphics[scale=0.25]{Screenshot (2269).png}

\noindent The $1$-skeleton of $X$ appears as in the diagram below on the left. 

\includegraphics[scale=0.4]{Screenshot (2271).png}

\noindent Since the subcomplex spanned by the path $a$ is contractible, the space is then homotopy equivalent to the space on the right, which is the wedge sum of $3$ circles. Hence $\pi_1(X^1)$ is a free group of rank $3$. Let $q$ be the quotient map. $q$ is then a homotopy equivalence, and so induces an isomorphism $q_*:\pi_1(X^1)\to\pi_1(S^1\vee S^1\vee S^1)$, whose inverse in particular maps bases to bases. $\{q_*([ab]),q_*([ac^{-1}]),q_*([ad])\}$ is a basis of $\pi_1(S^1\vee S^1\vee S^1)$, and hence $\{[ab],[ac^{-1}],[ad]\}$ is a basis of $\pi_1(X^1)$. To obtain $X^2$, we attach the three $2$-cells by the attaching maps $abcd,adb^{-1}c^{-1}$ and $ac^{-1}d^{-1}b$. So if we let $i:=[ab],j:=[ac^{-1}],k:=[ad]$, by Proposition $1.26$, we have that \[\pi_1(X)\cong\langle i,j,k\mid ij^{-1}k,ki^{-1}j,jk^{-1}i\rangle=\langle i,j,k\mid i=jk,j=ki,k=ij\rangle.\] This is a presentation of the quaternion group $Q_8$, as required.
\end{proof}
...
\end{cases}\]if $n>1$. $H_1$ is well-defined, since both $p(t)$ and $R_1(s,t)$ are equal to $(0,0,0)$ on $p^{-1}(\{(0,0,0)\})=A_1\cap(\bigcup_{i=2}^\infty A_i)$. And $H_n$ is well-defined $\forall n>1$, since the intersection of the preimages of any pairs of $\bigcup_{i=1}^{n-1}A_i,A_n$ and $\bigcup_{i=n+1}^\infty A_i$ is $p^{-1}(\{(0,0,0)\})$, and $H_{n-1}(1-\frac{1}{n+1},t),R_n(s,t)$ and $p(t)$ are all equal to $(0,0,0)$ on that set. Furthermore, both $A_1$ and $\bigcup_{i=2}^\infty A_i$ are closed, so $H_1$ is continuous by the pasting lemma, and $\bigcup_{i=1}^{n-1}A_i,A_n$ and $\bigcup_{i=n+1}^\infty A_i$ are closed, and hence $H_n$ is continuous by the pasting lemma for $n>1$. We then define $F:I\times I\to X$ by\[F(s,t)=\begin{cases}
...
\end{cases}\]This is continuous on $[0,1-\frac{1}{n+2}]\times I\forall n$ by the pasting lemma. Furthermore, $F(1,t)=(0,0,0)\forall t$, since if $p(t)\in A_n$ for some $n$, then $F(s,t)=(0,0,0)\forall s>1-\frac{1}{n+5}$. We now need to show that it is continuous on $\{1\}\times I$. Indeed, let $t\in I$ and let $\epsilon > 0$. Let $n\in\mathbb{N}$ be such that $\frac{2}{n}<\epsilon$. Then for all $(s',t')\in B_{\frac{1}{n+3}}((1,t))\cap (I\times I)$, we have \[F(s',t')\in\bigcup_{i=n+1}^\infty A_i\subseteq B_{\frac2{n+1}}((0,0,0))\subseteq B_{\epsilon}((0,0,0)),\] as required. We then have a based homotopy from $p$ to $\gamma_{(0,0,0)}$. Since $p$ is arbitrary, it follows that $X$ is simply-connected.
\end{proof}

\begin{exercise}[*]
Let $X$ be the subspace of $\mathbb{R}^2$ that is the union of the circles $C_n$ of radius $n$ and center $(n,0)$ for $n=1,2,\dots$. Show that $\pi_1(X)$ is the free group $*_n\pi_1(C_n)$, the same as for the infinite wedge sum $\bigvee_\infty S^1$. Show that $X$ and $\bigvee_\infty S^1$ are in fact homotopy equivalent, but not homeomorphic.
\end{exercise}
\begin{proof}
\includegraphics[scale=0.3]{Screenshot (2273).png}

\noindent Let $B_{\epsilon_n,n}\subseteq\mathbb{R}^2$ be an $\epsilon_n$-neighbourhood of $C_n$. For sufficiently small $\epsilon_n$, $C_n$ is the only circle in $X$ fully contained in $B_{\epsilon_n,n}$. Let $A_n=B_{\epsilon_n,n}\cap X\forall n$. Then $A_n$ is open, path-connected, and deformation retracts to $C_n$, and so in particular has fundamental group isomorphic to $\mathbb
{Z}$. Furthermore, each $A_n$ contains the basepoint $(0,0)$, and the intersection of any $A_i\cap A_j\cap A_k$ is path-connected. Furthermore, $A_i\cap A_j$ is simply-connected for $i\neq j$, since the only complete circle contained in $A_i$ is $C_i$, which is not contained in $A_j$, and ditto for $A_j$. Hence, the Van Kampen theorem applies to give \[\pi_1(X)\cong *_{i=1}^\infty\pi_1(A_i)/\{0\}\cong *_{i=1}^\infty\mathbb{Z},\] as required.
\end{proof}
\subsection{Covering Spaces}
\begin{exercise}
For a covering space $p:\overset{\sim}{X}\to X$ and a subspace $A\subset X$, let $\overset{\sim}{A}=p^{-1}(A)$. Show that the restriction $p:\overset{\sim}{A}\to A$ is a covering space.
\end{exercise}
\begin{proof}
Let $a\in A$. Then, since $a\in X$, there exists an open neighbourhood $U\subset X$ such that $p^{-1}(U)$ is a disjoint union of open sets $S_\alpha$ such that $p_{|S_\alpha}\to U$ is a homeomorphism $\forall\alpha$. We let $V:=U\cap A$, which is then open in $A$ by the subspace topology, while still containing $a$. Furthermore, \[p^{-1}(V)=p^{-1}(U\cap A)=p^{-1}(U)\cap p^{-1}(A)=\left(\bigsqcup_\alpha S_\alpha\right)\cap \overset{\sim}{A}=\bigsqcup_\alpha (S_\alpha\cap \overset{\sim}{A}).\]Finally, for any $\alpha$ we have an inverse $q_\alpha:U\to S_\alpha$ of $p_{|S_\alpha}$, so we consider $q_{\alpha|A}:A\to S_\alpha\cap\overset{\sim}{A}$. This is inverse to $p_{|S_\alpha\cap\overset{\sim}{A}}:S_\alpha\cap\overset{\sim}{A}\to A$, and hence $p_{|S_\alpha\cap\overset{\sim}{A}}$ is a homeomorphism from $S_\alpha\cap\overset{\sim}{A}$ to $V$. Hence, $p:\overset{\sim}{A}\to A$ is a covering space.
\end{proof}
\begin{exercise}
Show that if $p_1:\overset{\sim}{X_1}\to X_1$ and $p_2:\overset{\sim}{X_2}\to X_2$ are covering spaces, so is their product $p_1\times p_2:\overset{\sim}{X_1}\times\overset{\sim}{X_2}\to X_1\times X_2$.
\end{exercise}
\begin{proof}
Let $(x_1,x_2)\in X_1\times X_2$. Then there exists open sets $U_1\subseteq X_1$ and $U_2\subseteq X_2$ such that each $U_i$ contains $x_i$, and hence $(x_1,x_2)\in U_1\times U_2$. Furthermore, $U_1\times U_2$ is open, by the definition of the product topology. Additionally, we have $p_1^{-1}(U_1)=\bigsqcup_\alpha S_\alpha$ for open sets $S_\alpha$, each of which map homeomorphically onto $U_1$ by $p_1$, and $p_2^{-1}(U_2)=\bigsqcup_\beta T_\beta$ for open sets $T_\beta$, each of which map homeomorphically onto $U_2$ by $p_2$. Hence, \[(p_1\times p_2)^{-1}(U_1\times U_2)=p_1^{-1}(U_1)\times p_2^{-1}(U_2)=\bigsqcup_\alpha S_\alpha\times\bigsqcup_\beta T_\beta=\bigsqcup_{\alpha,\beta}S_\alpha\times T_\beta,\] where each $S_\alpha\times T_\beta$ is open by the product topology. Finally, for any $\alpha,\beta$, $(p_1\times p_2)_{|S_\alpha\times T_\beta}=p_{1|S_\alpha}\times p_{2|T_\beta}$ is a homeomorphism onto $U_1\times U_2$, being the product of two homeomorphisms. Hence, $p_1\times p_2:\overset{\sim}{X_1}\times\overset{\sim}{X_2}\to X_1\times X_2$ is a covering map.
\end{proof}
\begin{exercise}
Let $p:\overset{\sim}{X}\to X$ be a covering space with $p^{-1}(x)$ finite and nonempty for all $x\in X$. Show that $\overset{\sim}{X}$ is compact Hausdorff iff $X$ is compact Hausdorff.
\end{exercise}
\begin{proof}
$(\implies)$\textbf{We first prove that $X$ is compact.}

\noindent Let $\bigcup_\alpha U_\alpha$ be an open cover of $X$. $\bigcup_\alpha p^{-1}(U_\alpha)$ is then an open cover of $\overset{\sim}{X}$, and hence, by compactness, there exists a finite subcover $p^{-1}(U_{i_1}),\dots,p^{-1}(U_{i_n})$ of $\overset{\sim}{X}$. Since $p$ is surjective, due to $p^{-1}(x)$ being nonempty for all $x\in X$, it follows that $U_{i_1},\dots, U_{i_n}$ is an open cover of $X$. Hence, $X$ is compact.

\noindent\textbf{We now prove that $X$ is Hausdorff.}

\noindent Let $x,y$ be two distinct points in $X$. Let $A$ and $B$ be open neighbourboods of $x$ and $y$ respectively such that $p^{-1}(A)=\bigsqcup_{i=1}^nS_i$ and $p^{-1}(B)=\bigsqcup_{i=1}^mT_i$, where each $S_i$ maps homeomorphically onto $A$ by $p$, and ditto for $T_i$ onto $B$. Let $\{x_1,\dots, x_n\}$ and $\{y_1,\dots, y_m\}$ be the preimages of $x$ and $y$ under $p$ respectively, so that $x_i\in S_i\forall i$ and $y_j\in T_j\forall j$. Since $\overset{\sim}{X}$ is Hausdorff, there exist open sets $U_{i,j}\subseteq S_i$ and $V_{i,j}\subseteq T_j$ such that $x_i\in U_{i,j}$, $y_j\in V_{i,j}$, and $U_{i,j}\cap V_{i,j}=\emptyset$. Let $\mathcal{U}_i:=\bigcap_{j}U_{i,j}$ and let $\mathcal{V}_j:=\bigcap_{i}V_{i,j}$. These sets are open, since they are finite intersections of open sets. Hence, since $p$ is an open map, $\bigcap_ip(\mathcal{U}_i)$ and $\bigcap_jp(\mathcal{V}_j)$ are both open sets, where the former contains $x$, and the latter contains $y$. Suppose for a contradiction that their intersection is non-empty. Let $w$ be a point in the preimage of their intersection. Since $p$ maps each $S_i$ and $T_j$ homeomorphically onto $A$ and $B$ respectively, it follows that $w\in\mathcal{U}_i$ and $w\in\mathcal{V}_j$ for some $i,j$. But then $w\in U_{i,j}\cap V_{i,j}$; a contradiction. Hence, $X$ is Hausdorff.

\noindent $(\impliedby)$\textbf{We first prove that $\overset{\sim}{X}$ is compact.}

\noindent Let $\bigcup_\alpha U_\alpha$ be an open cover of $\overset{\sim}{X}$. Given $x\in X$, let $U_x$ be an evenly-covered neighbourhood of $x$. Let $\{y_1,\dots, y_{n}\} = p^{-1}(x)$, and let $S_1,\dots, S_{n}$ be the sheets of $U_x$, such that $y_i\in S_i\forall i$. Given each $y_i$, there exists an $\alpha_i$ such that $y_i\in U_{\alpha_i}$. Let $V_i:=U_{\alpha_i}\cap S_i$, and let $W_x:=\bigcap_{i=1}^np(V_i)$, which is open, being the finite intersection of open sets. For each $i$, we have $p^{-1}(W_x)\cap S_i=p_{|S_i}^{-1}(W_x)\subseteq p_{|S_i}^{-1}(p(U_{\alpha_i}\cap S_i))\subseteq U_{\alpha_i}$. Hence, since $W_x\subseteq U_x$, implying that $p^{-1}(W_x)\subseteq \bigcup_{i=1}^n S_i$, and hence that $p^{-1}(W_x)=\bigcup_{i=1}^n p^{-1}(W_x)\cap S_i$, it follows that $p^{-1}(W_x)\subseteq \bigcup_{i=1}^n U_{\alpha_i}$. Since $\bigcup_{x\in X}W_x$ is an open cover of $X$, by compactness, it follows that there exists a finite subcover $\bigcup_{\lambda=1}^m W_{x_\lambda}$. Since each $p^{-1}(W_x)$ is a subset of a finite union of $U_\alpha$'s, we then obtain a finite subcover of $\overset{\sim}{X}$.

\noindent\textbf{We now prove that $\overset{\sim}{X}$ is Hausdorff.}

\noindent Let $x,y$ be two distinct points in $\overset{\sim}{X}$. If $p(x)=p(y)$, then it follows that $x$ and $y$ are contained in different disjoint open sheets, as required. So we can assume that $p(x)\neq p(y)$. In this case, since $X$ is Hausdorff, we have open neighbourhoods $U$ and $V$ of $p(x)$ and $p(y)$ respectively so that $U\cap V=\emptyset$. $p^{-1}(U)$ and $p^{-1}(V)$ are then disjoint open neighbourhoods of $x$ and $y$ respectively, as required. Hence, $\overset{\sim}{X}$ is Hausdorff.
\end{proof}
\end{document}
