...
\end{exercise}
\begin{proof}
By Exercise $0.20$, we have that $X$ is homotopy equivalent to $S^1\vee S^1\vee S^2$, and so has fundamental group $\mathbb{Z}*\mathbb{Z}$ by Proposition $1.26$. We can obtain $X$ from $Y$ by attaching a single $2$-cell, and so by Proposition $1.26$, the inclusion map $Y\hookrightarrow X$ induces a surjection $\pi_1(Y)\to\pi_1(X)$ with kernel the normal subgroup generated by the attaching map.
\end{proof}
