...
\end{exercise}
\begin{proof}
First, assume that $X$ is connected. Without loss of generality, we can assume that none of the edges of $X$ intersect, for if they do, then we can simply add a vertex at the point of intersection. We begin by considering the subgraph $Y$ consisting of all vertices of $X$ which do not lie on $P$, and then all edges which do not connect to a point on $P$. Let $V:=\{v_1,...,v_n\}$ be the set of vertices, with $v_1$ being connected to a vertex on $P$ by an edge, let $T$ be a spanning tree of $Y$, and let $Q$ be the collection of edges which are not part of $T$, along with their vertices, which we shall denote as $A:=\{w_{i(1)},...,w_{i(m)}\}$. We then have that $Y\cong T\sqcup_f Q$, with $f:A\to T:w_{i(k)}\mapsto v_{i(k)}$. Now let $g:A\to T:w_{i(k)}\mapsto v_1$ send every vertex of $Q$ to $v_1$. Clearly $f\simeq g$, and hence $Y\simeq T\sqcup_g Q$. Let $F:A\times I\to T$ be a homotopy from $f$ to $g$. $F$ then induces a homotopy $G:A\times I\times S^1\to T\times S^1:(w_{i(k)},t,\omega)\mapsto(F(w_{i(k)},t),\omega)$. $\pi_T\circ G(w_{i(k)},t,\omega)=F(w_{i(k)},t)$ and $\pi_{S^1}\circ G(w_{i(k)},t,\omega)=\omega$, and hence $G$ is continuous by the universal property of the product topology. Let $\alpha(w_{i(k)},\omega)=G(w_{i(k)},0,\omega)$ and let $\beta(w_{i(k)},\omega)=G(w_{i(k)},1,\omega)$, so that $\alpha$ maps the circles in $A\times S^1$ in the normal way to obtain $Y\times S^1$, while $\beta$ maps every circle in $A\times S^1$ to $\{v_1\}\times S^1$. We then have $Y\times S^1\cong T\times S^1\sqcup_\alpha Q\times S^1\simeq T\times S^1\sqcup_\beta Q\times S^1$. We can then deformation retract $T\times S^1$ to a circle, and hence $Y\times S^1\simeq S^1\times\bigvee_{i=1}^n S^1$, where $n$ is the number of edges in $Y$ which are not part of $T$.

We now consider the other part of $X$ - namely, the set of vertices which lie on $P$, the set of vertices which are connected to a point on $P$ by an edge, and all edges which have an endpoint on $P$. Call this subgraph $Z$. Similarly to above, we can homotope all the points on $Z$ not on $P$ to $v_1$, and so deduce that $X$ is homotopy equivalent to $S\times(\bigvee_{i=1}^n S^1\sqcup_q Z)$, where $q$ sends every vertex of $Z$ to $v_1$.
