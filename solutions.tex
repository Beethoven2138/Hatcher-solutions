...
\end{exercise}
\begin{proof}
Let $p:I\to X$ be a path. For each $t\in I$, let $U_t$ be a star-shaped open neighbourhood of $p(t)$. By compactness (since the continuous image of a compact set is compact), there exists a finite subset $J\in I$ such that $\bigcup_{t\in J}U_t\cap p(I)$ is an open cover of $p(I)$. Suppose $J=\{t_1,...,t_n\}$, where $t_1<\dots<t_n$. Let $x_0,...x_n$ be points in $U_{t_1},...,U_{t_n}$ such that the line segment between every point $x\in U_{t_i}$ and $x_i$ is contained in $U_{t_i}$. Let $V_t$ be the collection of the connected components of $U_{t_1}\cap p(I),...,U_{t_n}\cap p(I)$. Each $V_t$ is open in $p(I)$, since a connected component of an open set is open, and hence by compactness of $p(I)$ we can find a finite collection $V_{s_1},...,V_{s_m}$, ordered in increasing order of appearance on $p(I)$ (that is, $\inf p^{-1}(V_{s_i})\leq\inf p^{-1}(V_{s_j})\forall i,j:i<j$), which form an open cover of $p(I)$. Furthermore, we may assume without loss of generality that there do not exist $i,j$ such that $V_{s_i}\subseteq V_{s_j}$. We then construct a list of triples of the form $I\times I\times \{1,...,n\}$ in the following way: We begin with the first triple $A_1=(0,b_1,i_1)$, where $i$ is chosen such that $V_{s_1}$ is a connected component of $U_{t_i}$, and where $b_i$ is a point on $V_{s_1}\cap V_{s_2}$. Then, for each $A_{i}$ where $A_i(2)\neq 1$, we define $A_{i+1}$ by $A_{i+1}(1)=A_i(2)$, $A_{i+1}(3)$ as being such that $V_{s_{i+1}}$ is a connected component of $U_{t_{A_{i+1}(3)}}$, and $A_{i+1}(2)$ as some $t$ such that $p(t)\in V_{s_{i+1}}\cap V_{s_{i+2}}$ if $i+1<m$, and as $1$ otherwise. We can then construct a piecewise linear path $q:I\to X$ as follows: For each $i$, we let $q(A_i(1))=p(A_i(1))$, $q\left(\frac{A_i(1)+A_i(2)}{2}\right)=x_{A_i(3)}$, $q(A_i(2))=p(A_i(2))$, and then let $q$ draw out the line segment from $q(A_i(1))$ and $x_{A_i(3)}$ between $A_i(1)$ and $\frac{A_i(1)+A_i(2)}{2}$, and then the line segment from $x_{A_i(3)}$ to $q(A_i(2))$ between $\frac{A_i(1)+A_i(2)}{2}$ and $A_i(2)$. We can then form a based homotopy from $q_{|[A_i(1),A_i(2)]}$ to $p_{|[A_i(1),A_i(2)]}$ as follows: Let $h_t:[A_i(1),A_i(2)]\to X$ be the straight-line homotopy from $p_{|[A_i(1),A_i(2)]}$ to $\gamma_{x_{A_i(3)}}$ (the constant path), let $a_t:[A_i(1),A_i(2)]\to X$ be the path tracing out the line segment from $p(A_i{(1)})$ to $h_t(A_i(1))$, and let $b_t:[A_i(1),A_i(2)]\to X$ be the path tracing out the line segment from $h_t(A_i(2))$ to $p(A_i(2))$. Then $a_t\cdot h_t\cdot b_t$ is a homotopy from $q_{|[A_i(1),A_i(2)]}$ to $p_{|[A_i(1),A_i(2)]}$, as required. Doing this for every $i$ shows that $p$ and $q$ are based-homotopic. We can then reparametrize $q$ by arc-length, as required.

When $X$ is open, every $x\in X$ is contained in a ball, which is star-shaped, and hence $X$ is locally star-shaped.

When $X$ is a union of finitely many closed convex sets $C_1,...,C_n$, for each $x\in X$, we can 
\end{proof}
\end{document}
